...
Mock policy: mock action probabilities are synthetic and labeled; displayed elapsed time measures the local mock call including its deliberate delay, never vendor API latency. Ambiguous observations reduce mock confidence; scenario stage/time never directly invokes the planner. The confidence threshold requests planning independently of execution. Jev continues local control while System 2 works; low-confidence actions are marked provisional. At most one decision and one planner request per agent are in flight, each with timeout and stale-result guards. Planner cooldown prevents repeated requests on the same unresolved context. Jev sees neutral, unsorted per-action kinematic projections from sensed geometry; code never labels a safest/recommended action. Ineffective actions are omitted. Planner narrative is excluded from Jev input to prevent stale bearings from becoming steering instructions.
