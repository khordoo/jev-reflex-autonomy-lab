    ...
    questions: {
      action: {
        ...
        instructions: {
          ...
          traffic_policy:
            'DRONE contacts are other moving aircraft. Separate from them using relative velocities and projected clearance just like obstacles: every DRONE contact must clear by a safe margin, including parallel or co-speed aircraft. Overtaking rule: the leading aircraft keeps its course and has right of way; a faster aircraft catching up from behind must pass on the LEFT (counter-clockwise, negative-bearing side) of the slower aircraft, and slow down if the left side is not clear — never cut across another aircraft. For a head-on encounter prefer TURN_RIGHT when safe. Arrived drones are docked and leave the active flight lane.',
          arrival_policy:
