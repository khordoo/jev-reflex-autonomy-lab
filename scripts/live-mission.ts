...
const config = (await fetch('/api/providers').then((r) => r.json())) as {
  jevConfigured: boolean;
  plannerConfigured: boolean;
  plannerModel: string;
};
if (!config.jevConfigured || !config.plannerConfigured)
  throw new Error('Both live providers must be configured');
...
await new Promise<void>((resolve) => {
  const timer = setInterval(() => {
    ...
    if (world.time - lastReport >= 8) {
      lastReport = world.time;
      console.log(
        JSON.stringify({
          time: +world.time.toFixed(1),
          decisions: state.telemetry.length,
          confidence: state.decision?.confidence,
          action: state.decision?.action,
          planning: state.planning,
          strategyRevision: state.strategy.revision,
          health: world.agents.drone_001.health,
          error: state.error || state.plannerError,
        }),
      );
    }
    if (
...
while (
  (state.decisionPending || state.planning) &&
  performance.now() - drainStarted < 31000
)
  await new Promise((r) => setTimeout(r, 30));
controller.dispose();
const sortedLatencies = state.telemetry
  .map((e) => e.latencyMs)
  .sort((a, b) => a - b);
const summary = {
  ...
  confidenceMax: Math.max(...state.telemetry.map((e) => e.decision.confidence)),
  medianApiMs: sortedLatencies[Math.floor(sortedLatencies.length / 2)],
  completedPlans: state.planningEvents.filter((e) => e.status === 'completed')
    .length,
  failedPlans: state.planningEvents.filter((e) => e.status === 'failed').length,
  providerFailures: controller.failures.length,
  health: world.agents.drone_001.health,
  missionComplete: world.agents.drone_001.complete,
  collisions: world.agents.drone_001.collisions.length,
};
